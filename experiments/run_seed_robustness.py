...
def evaluate_for_seed(
    ...
    for policy in policies:
        ...
        rows.append(summarize_seed(policy.name, seed, losses, terminal_inventory, violations))
    return rows
...
def main() -> None:
    ...
    raw_table_path = output_dir / "tables" / "seed_robustness_raw.csv"
    aggregate_table_path = output_dir / "tables" / "seed_robustness_metrics.csv"
    aggregate_table_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    plot_metric(
        summary,
        "cvar_95_bps",
        output_dir / "figures" / "seed_robustness_cvar.png",
    ...
    plot_metric(
        summary,
        "mean_shortfall_bps",
        output_dir / "figures" / "seed_robustness_mean_shortfall.png",
    ...
    print(f"\nSaved raw table to {raw_table_path.relative_to(ROOT)}")
    print(f"Saved aggregate table to {aggregate_table_path.relative_to(ROOT)}")
